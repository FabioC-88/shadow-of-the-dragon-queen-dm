# Scontri Adattati — Capitolo 4: Shadow of War

**Copre:** Sessioni 03-08 (Liv. 4 → 6)
**Framework di difficoltà:** vedi [00-framework.md](00-framework.md) — CR budget *Flee, Mortals!*, niente regola Minion, Villain Actions solo per boss veramente seri o di fine capitolo
**Party:** 5 personaggi · **Letalità target:** Deadly/Hard scalato su 5 PG + azioni extra sui boss veri, fascia bassa preferita — vedi [regole-opzionali.md](../regole-opzionali.md) (critici massimizzati e riposo lungo limitato ai Dadi Vita alzano la letalità reale oltre il budget CR nudo)
**Lingua:** mostri, stat block e nomi delle azioni in **inglese**; tutto ciò che va letto ai giocatori resta in **italiano**.

> **Nota onestà DM:** valori ricalibrati a giudizio, non playtestati — stesso approccio di Cap. 3. `[TODO DM: aggiustare HP/DC dopo il primo turno reale al tavolo]`.
>
> **Nota ricontrollo (regole opzionali + stile Villain Actions):** [regole-opzionali.md](../regole-opzionali.md) è attivo da questa sessione — critici massimizzati e niente guarigione automatica dal riposo lungo rendono Caradoc e Knight Sarlamir più duri di quanto suggerisca il CR nudo, specie Sarlamir (HP 170, già alzato per il party a 5). `[TODO DM: verificare]` al tavolo. Rilette le Villain Actions di entrambi contro quelle di Fewmaster Gholcag (Cap. 3): nessun riciclo letterale, ma due echi strutturali da tenere d'occhio nei prossimi capitoli — il guadagno di PF temporanei sul colpo (Gholcag Ultimate, Caradoc Control) e l'AoE con tiro salvezza o prono (Gholcag Control, Sarlamir Ultimate). Diverso slot, trigger e tono narrativo in ogni caso (non è un copia-incolla), quindi non riscritto — ma è il tipo di ripetizione da evitare nei boss di Cap. 5-7.

---

## Panoramica del capitolo — chi ottiene cosa

Sei sessioni, molti nemici nominati, ma solo **due boss veri** secondo il criterio del framework ("è il motivo per cui la sessione/il capitolo esiste, o un ostacolo lungo la strada?"):

| Sessione | Livello | Boss vero (Villain Actions) | Minacce notevoli (Bonus Action + Reaction, niente Villain Actions) | Mook (stat block standard, contano solo i numeri) |
|---|---|---|---|---|
| 03 — L'Ombra della Guerra | 4 | — | — | nessun combattimento |
| 04 — La Prima Missione | 4 | — | Hobgoblin (capo) | 12× Goblin, stat block classico (iniziativa condivisa) |
| 05 — Le Vedette Perdute | 4 | — | Kapak Draconian (ambusher) | 3× Baaz Draconian |
| 06 — Le Mura di Wheelwatch | 5 | — | Sivak Draconian, Dragon Army Officer + Dragonnel (duo) | Dragon Army Soldier (gruppi da 1-8 per stanza, stat block classico anche se convergono) |
| 07 — Steel Springs / La Notte del Cavaliere Morto | 5 | **Caradoc** (2 fasi) | — | Dragon Army Soldier a cavallo, Undead Soldier |
| 08 — Le Catacombe / Sarlamir | 5→6 | **Knight Sarlamir** | — | Wraith, Skeletal Horse (entrambi opzionali/evitabili) |

---

## Sessione 03 — L'Ombra della Guerra (Livello 4)

Nessuno scontro strutturato — sessione di arrivo/roleplay a Kalaman. Nessun adattamento necessario.

---

## Sessione 04 — La Prima Missione (Livello 4)

### L'Officina Assediata

**12× Goblin (stat block classico)** + **1× Hobgoblin (capo)** — era 10+1, scalato per il quinto giocatore. Niente regola Minion: sono 12 goblin individuali, gestiti con **iniziativa condivisa** (una sola tirata per il gruppo, vedi 00-framework.md punto 2) per non rallentare il tavolo. La tabella ambientale **Clockwork Chaos** (i marchingegni impazziti di Rookledust) resta **invariata** — è già un'ottima meccanica di scena, non toccarla.

```
GOBLIN (×12)
Small Humanoid | AC 15 (leather armor, shield) | HP 7 (2d6)
Speed 30 ft | CR 1/4 (50 XP each) | Languages: Common, Goblin

Skills: Stealth +6 | Darkvision 60 ft, Passive Perception 9

Nimble Escape: as a bonus action, can Disengage or Hide.

ACTIONS
Scimitar: +4 to hit, reach 5 ft — 5 (1d6+2) slashing
Shortbow: +4 to hit, range 80/320 ft — 5 (1d6+2) piercing
```

*Nota tavolo: con 12 corpi in gioco, l'iniziativa condivisa è quasi obbligatoria per il ritmo — tira una sola volta per l'intero gruppo di Goblin, muovili/attaccali tutti nello stesso momento del round. Il sapore "goblin agile" resta nella Nimble Escape standard, non in una meccanica di gruppo custom.*

```
HOBGOBLIN — CAPO (Notable, ~CR 1/2)
Medium Humanoid | AC 18 (chain mail, shield) | HP 15
Speed 30 ft | Lawful Evil | Languages: Common, Goblin

Darkvision 60 ft | Passive Perception 10

Martial Advantage: once per turn, deals an extra 7 (2d6) damage to a creature it hits
with a weapon attack if a Hobgoblin ally is within 5 ft of the target and not incapacitated.

ACTIONS
Longsword: +3 to hit, reach 5 ft — 5 (1d8+1) slashing, or 6 (1d10+1) two-handed,
  + 7 (2d6) if Martial Advantage applies.
Longbow: +3 to hit, range 150/600 ft — 5 (1d8+1) piercing.

REACTION — Cowardly Retreat
When damage brings him below half his HP, he can use his reaction to Disengage
and move away. Coerente con l'originale: fugge se in inferiorità numerica.
```

*Non è un boss di capitolo — niente Villain Actions. La sua unica "mossa speciale" è scappare bene, non colpire più forte.*

---

## Sessione 05 — Le Vedette Perdute (Livello 4)

### Il Posto di Vedetta: i Primi Draconiani

**3× Baaz Draconian** (era 2, scalato per il quinto giocatore) + **1× Kapak Draconian (Ambusher)**. Stat block classici, come sempre in questa campagna.

> ⚠️ **[TODO DM — ricontrollare il budget di questo scontro.]** Il Baaz qui era scritto come **CR 1 (200 XP)**; la scheda ufficiale è **CR 1/2 (100 XP)**, con danni molto più bassi (2× Shortsword 1d6+1 = ~8/round, invece di Morso+Artigli o 2× Longsword = ~11-14/round). Il budget *Flee, Mortals!* di questo incontro era calcolato sui numeri vecchi: **i tre baaz ora pesano circa la metà.** Se vuoi mantenere la pressione originale, alza il numero (4-5 baaz) invece di gonfiare il blocco. Vale per tutti gli scontri con baaz dal Cap. 4 in poi.

```
BAAZ DRACONIAN (×3) — DSotDQ p197, scheda ufficiale (verificata 2026-09-13)
Medium Monstrosity, typically Lawful Evil | AC 14 (natural armor)
Initiative +0 (10) | HP 22 (4d8+4) | Speed 30 ft.
STR 13 (+1, save +1)   DEX 11 (+0, save +0)   CON 13 (+1, save +1)
INT  8 (-1, save -1)   WIS  8 (-1, save -1)   CHA 10 (+0, save +0)
Gear Shortsword | Senses Darkvision 60 ft., Passive Perception 9
Languages Common, Draconic | CR 1/2 (XP 100; PB +2)

TRAITS
Controlled Fall. When the draconian falls and isn't incapacitated, it subtracts
  up to 100 ft from the fall when calculating the fall's damage.
  [NON vola e NON plana lateralmente: attutisce solo i danni da caduta.]
Death Throes. When reduced to 0 HP, its body turns to stone and releases a
  petrifying gas. Each creature within 5 ft must succeed on a DC 11 Constitution
  save or be restrained as it begins to turn to stone. The restrained creature
  repeats the save at the end of its next turn: on a success the effect ends,
  otherwise the creature is PETRIFIED for 1 minute. After 1 minute, the body
  crumbles to dust.
Draconic Devotion. While it can see a Dragon that isn't hostile to it, the
  draconian has advantage on attack rolls.

ACTIONS
Multiattack. Two Shortsword attacks.
Shortsword. +3 to hit, reach 5 ft — 4 (1d6+1) piercing.
```

```
KAPAK DRACONIAN (Ambusher) — base ufficiale DSotDQ p198 (verificata 2026-09-13)
Medium Monstrosity, typically Lawful Evil | AC 15 (natural armor)
Initiative +3 (13) | HP 39 (6d8+12) | Speed 40 ft., Climb 40 ft.
STR 11 (+0, save +0)   DEX 17 (+3, save +5)   CON 14 (+2, save +2)
INT 12 (+1, save +1)   WIS 13 (+1, save +1)   CHA 11 (+0, save +0)
Skills Deception +4, Perception +3, Stealth +7
Immunities Poison; Poisoned | Gear Dagger
Senses Darkvision 60 ft., Passive Perception 13
Languages Common, Draconic | CR 3 (XP 700; PB +2)

TRAITS
Death Throes. When reduced to 0 HP, it dissolves into acid that splashes on those
  around it. Each creature within 5 ft must succeed on a DC 12 Dexterity save or
  be covered in acid for 1 minute, taking 7 (2d6) acid damage at the start of each
  of its turns. A creature can use its action to scrape or wash the acid off
  itself or another creature.
Glide. When it falls and isn't incapacitated, it subtracts up to 100 ft from the
  fall when calculating damage, and can move up to 2 ft horizontally for every
  1 ft it descends. NOT flight: it cannot gain altitude.

ACTIONS
Multiattack. Two Dagger attacks. If both hit the same creature, that target must
  succeed on a DC 12 Constitution save or be poisoned until the end of its next
  turn. While poisoned this way, the target is also PARALYZED.
Dagger. Melee or Ranged: +5 to hit, reach 5 ft. or range 20/60 ft.
  Hit: 5 (1d4+3) piercing plus 7 (2d6) poison.

--- AGGIUNTE HOMEBREW (dichiarate, come nel Cap. 3) ---
Sneak Attack: +2d6 se ha vantaggio o un alleato è entro 5 ft dal bersaglio.
REACTION — Glide Away: quando un attacco in mischia lo manca, può muoversi di
  20 ft senza provocare attacchi di opportunità.

[NOTA: era scritto come Medium Dragon CR 2, 45 PF, con Shortsword/Light Crossbow
 e "Poisoned Blades" come azione bonus. Niente di tutto ciò è ufficiale: è una
 Mostruosità CR 3 con un Pugnale, il veleno è sempre attivo, e il Multiattacco
 può PARALIZZARE. Rivedere il budget dell'incontro di conseguenza.]
```

*Notevole ma non boss: niente Villain Actions. Il picco di difficoltà della sessione resta l'imboscata dall'alto (sorpresa + veleno), non un macchinario narrativo a tre atti.*

---

## Sessione 06 — Le Mura di Wheelwatch (Livello 5)

Infiltrazione, non un singolo scontro: il forte ha nove aree (W1-W9) con gruppi di Dragon Army Soldier da 1 a 8 per stanza. Niente regola Minion — se il tavolo fa scattare l'allarme totale e più gruppi convergono nello stesso scontro (es. gli 8 della caserma + i 3 del cortile = 11+), restano lo stesso stat block classico sotto, gestiti con **iniziativa condivisa** per non rallentare il tavolo (vedi 00-framework.md punto 2).

```
DRAGON ARMY SOLDIER (gruppi da 1-8 per stanza, o di più se convergono) — DSotDQ p200, official stat block
Medium Humanoid, typically Lawful Evil | AC 17 (scale mail, shield)
Initiative +1 (11) | HP 22 (4d8+4) | Speed 30 ft.
STR 15 (+2, save +2)   DEX 12 (+1, save +1)   CON 12 (+1, save +1)
INT 10 (+0, save +0)   WIS 10 (+0, save +0)   CHA 10 (+0, save +0)
Skills Athletics +4, Perception +2 | Gear Javelin, Longsword
Senses Passive Perception 12 | Languages Common, Draconic
CR 1 (XP 200; PB +2)

TRAITS
Draconic Devotion. While it can see a Dragon that isn't hostile to it, the
  soldier has advantage on attack rolls.

ACTIONS
Multiattack. Two Longsword or Javelin attacks.
Longsword. +4 to hit, reach 5 ft — 6 (1d8+2) slashing, or 7 (1d10+2) two-handed,
  plus 2 (1d4) fire.
Javelin. Melee or Ranged: +4 to hit, reach 5 ft. or range 30/120 ft.
  Hit: 5 (1d6+2) piercing plus 2 (1d4) fire.
```

```
DRAGON ARMY OFFICER (Notable, ~CR 3) — DSotDQ p200, official stat block
Medium Humanoid, typically Lawful Evil | AC 19 (splint, shield)
Initiative +2 (12) | HP 65 (10d8+20) | Speed 30 ft.
STR 16 (+3, save +3)   DEX 14 (+2, save +4)   CON 15 (+2, save +2)
INT 12 (+1, save +1)   WIS 14 (+2, save +4)   CHA 12 (+1, save +1)
Skills Athletics +5, Perception +4 | Gear Heavy Crossbow, Lance
Senses Passive Perception 14 | Languages Common, Draconic
CR 3 (XP 700; PB +2)

TRAITS
Draconic Devotion. While it can see a Dragon that isn't hostile to it, the
  officer has advantage on attack rolls.

ACTIONS
Multiattack. Two Vicious Lance attacks, and uses Assault Orders if available.
Vicious Lance. +5 to hit, REACH 10 ft — 8 (1d10+3) piercing plus 2 (1d4) fire.
Heavy Crossbow. +4 to hit, range 100/400 ft — 7 (1d10+2) piercing plus 5 (1d10) fire.
Assault Orders (Recharge 5-6). Shouts orders at up to two other creatures within
  60 ft. A target with Draconic Devotion that can hear it may use its reaction to
  make one melee attack.
```

```
SIVAK DRACONIAN (Notable, ~CR 4) — DSotDQ p199, official stat block
LARGE Monstrosity, typically Lawful Evil | AC 16 (natural armor)
Initiative +0 (10) | HP 57 (6d10+24) | Speed 30 ft., Fly 60 ft.
STR 18 (+4, save +6)   DEX 10 (+0, save +0)   CON 18 (+4, save +4)
INT 13 (+1, save +1)   WIS 10 (+0, save +2)   CHA 10 (+0, save +0)
Senses Darkvision 60 ft., Passive Perception 10
Languages Common, Draconic | CR 4 (XP 1100; PB +2)

TRAITS
Death Throes. When reduced to 0 HP by a Large or smaller creature, it crumbles
  into dust that forms a spectral, shrieking image OF ITS KILLER, lasting 1 minute.
  Each creature hostile to the draconian within 10 ft of the image: DC 14 Wisdom
  save or be frightened of it for 1 minute (repeat save at end of each turn).

ACTIONS
Multiattack. Two Serrated Sword attacks and one Tail attack.
Serrated Sword. +6 to hit, reach 5 ft — 13 (2d8+4) slashing.
Tail. +6 to hit, reach 5 ft — 8 (1d8+4) bludgeoning. If the target is Large or
  smaller, DC 14 Strength save or knocked prone.

REACTIONS
Shape Theft. After killing a Medium or smaller Humanoid, the draconian cloaks
  itself in an illusion to look and feel like that creature, keeping its own
  statistics (other than size). Lasts until it dies or ends it as a bonus action.
```

*Il vero peso di questo incontro è la rivelazione (parla come un alleato finché non lo colpisci), non il numero di azioni — niente Villain Actions, ma la Bonus Action rinforza il tema dell'inganno.*

```
DRAGON ARMY DRAGONNEL (Notable duo con l'Officer, CR 3 ufficiale — DSotDQ p201, official stat block
Large Dragon, typically Lawful Evil | AC 16 (breastplate barding)
Initiative +2 (12) | HP 58 (9d10+9) | Speed 30 ft., Fly 60 ft.
STR 16 (+3, save +3)   DEX 15 (+2, save +2)   CON 12 (+1, save +1)
INT  8 (-1, save -1)   WIS 13 (+1, save +1)   CHA 10 (+0, save +0)
Skills Perception +3 | Resistances Fire
Senses Blindsight 30 ft., Darkvision 120 ft., Passive Perception 13
Languages understands Common and Draconic but can't speak
CR 3 (XP 700; PB +2)

TRAITS
Flyby. Doesn't provoke opportunity attacks when it flies out of an enemy's reach.

ACTIONS
Multiattack. Two Rend attacks.
Rend. +5 to hit, reach 5 ft — 10 (2d6+3) slashing plus 3 (1d6) fire.
```

*Il suo cavaliere è un secondo **Dragon Army Officer** (blocco sopra) — la coppia condivide già il Comando dell'Officer come strumento di sinergia, niente bisogno di altro. Duo drammatico, non boss di capitolo: niente Villain Actions piene.*

**[Correzione]** Il Dragonnel ufficiale è **CR 3** (HP/danno base più bassi, e ha il tratto **Flyby** che qui mancava — aggiunto sopra). Ho lasciato HP e danno alzati a livello ~CR4: è una scelta dichiarata per far pesare il duo con l'Officer nella scena, non un errore di ricostruzione — segnalo qui per trasparenza.

---

## Sessione 07 — Steel Springs e la Notte del Cavaliere Morto (Livello 5)

### La Battaglia del Guado (Steel Springs)

**5× Dragon Army Soldier a cavallo** (era 4, scalato) + **Warhorses** vs 6 Kalaman Soldier feriti (alleati in ritirata, PF ridotti). La tabella eventi del campo di battaglia (d8) resta **invariata** — è già ottima, guidata dalla fonte.

```
KALAMAN SOLDIER (Wounded, allies) (×6)
Medium Humanoid | AC 14 (studded leather) | HP 8 (already wounded; full value 11) | Speed 30 ft

ACTIONS
Shortsword: +3 to hit, reach 5 ft — 4 (1d6+1) piercing.
```

```
DRAGON ARMY SOLDIER (mounted) (×5) — DSotDQ p200, official stat block
Medium Humanoid, typically Lawful Evil | AC 17 (scale mail, shield)
Initiative +1 (11) | HP 22 (4d8+4) | Speed 30 ft.
STR 15 (+2, save +2)   DEX 12 (+1, save +1)   CON 12 (+1, save +1)
INT 10 (+0, save +0)   WIS 10 (+0, save +0)   CHA 10 (+0, save +0)
Skills Athletics +4, Perception +2 | Gear Javelin, Longsword
Senses Passive Perception 12 | Languages Common, Draconic
CR 1 (XP 200; PB +2)

TRAITS
Draconic Devotion. While it can see a Dragon that isn't hostile to it, the
  soldier has advantage on attack rolls.

ACTIONS
Multiattack. Two Longsword or Javelin attacks.
Longsword. +4 to hit, reach 5 ft — 6 (1d8+2) slashing, or 7 (1d10+2) two-handed,
  plus 2 (1d4) fire.
Javelin. Melee or Ranged: +4 to hit, reach 5 ft. or range 30/120 ft.
  Hit: 5 (1d6+2) piercing plus 2 (1d4) fire.
```

```
WARHORSE (×5) — MM'25 p373, scheda ufficiale (verificata 2026-09-13)
Large Beast, Unaligned | AC 11 | Initiative +1 (11)
HP 19 (3d10+3) | Speed 60 ft. | CR 1/2 (XP 100; PB +2)
STR 18 (+4, save +4)   DEX 12 (+1, save +1)   CON 13 (+1, save +1)
INT  2 (-4, save -4)   WIS 12 (+1, save +3)   CHA  7 (-2, save -2)
Senses Passive Perception 11 | Languages —

ACTIONS
Hooves. Melee Attack: +6, reach 5 ft. Hit: 9 (2d4+4) Bludgeoning damage.
  If the target is a Large or smaller creature and the horse moved 20+ ft
  straight toward it immediately before the hit, the target takes an extra
  5 (2d4) Bludgeoning damage and has the Prone condition.
  [AC 11 non 10; danno base 2d4+4 non 2d6+4; la carica e' dentro l'attacco.]
```

*Obiettivo dello scontro invariato: proteggere la ritirata, non vincere una battaglia campale. Nessun trattamento boss — è un incontro tattico corale, non un nemico nominato.*

### I Guardiani del Cortile (Castle Kalaman)

**3× Undead Soldier** (era 2, scalato) — falsi Cavalieri di Solamnia, bloccano l'ingresso.

```
UNDEAD SOLDIER (×3)
Medium Undead | AC 18 (plate armor) | HP 45 (6d8+18)
Speed 30 ft | CR 3 (700 XP each) | Lawful Evil

STR +2, DEX +2, CON +3, INT +0, WIS +1, CHA +2
Skills: Perception +3, Stealth +4
Resistant to necrotic, bludgeoning/piercing/slashing from nonmagical attacks
Immune to poison damage, exhaustion, poisoned condition
Darkvision 60 ft, Passive Perception 13

ACTIONS
Multiattack: two attacks, only one can be Life Drain.
Life Drain: +4 to hit, reach 5 ft — 5 (1d6+2) necrotic; target's HP maximum reduces
by the damage dealt (until it finishes a long rest), and the Undead Soldier regains
HP equal to the damage dealt. If this reduces the target's HP maximum to 0, it dies
and rises in 24 hours as a zombie under the soldier's control, unless the body is
destroyed or blessed.
Longsword: +4 to hit, reach 5 ft — 6 (1d8+2) slashing, or 7 (1d10+2) two-handed.
```

*Guardiani muti, non nominati — nessun trattamento speciale. La minaccia è il Life Drain, non l'economia delle azioni.*

### Caradoc — Boss di Capitolo (2 Fasi)

Questo è il vero cuore del capitolo: il momento in cui il party scopre il nome **Lord Loren Soth**. Trattamento pieno — ma **solo nella Fase 2**, quando Caradoc smette di fingere di essere mortale. Nella Fase 1 è ancora "solo" un cavaliere posseduto: Bonus Action e Reaction bastano a renderlo pericoloso senza anticipare la sua vera natura.

**Personalità che guida il design:** Caradoc è vanitoso, codardo, teatrale — vive per il pubblico, non per la vittoria. A differenza di Gholcag (fame/forza bruta), le sue azioni sono tutte su paura, inganno e fuga.

```
CARADOC — POSSESSED KNIGHT (Fase 1 — Notable, ~CR 3)
Medium Humanoid (any alignment) | AC 18 (plate armor) | HP 65
Speed 30 ft | Languages: Common

STR +3, DEX +0, CON +2, INT +0, WIS +0, CHA +2
Saves: CON +4, WIS +2 | Skills: Persuasion +4 | Passive Perception 10

Brave: advantage on saving throws against being frightened.

ACTIONS
Multiattack: two melee attacks.
Greatsword: +5 to hit, reach 5 ft — 10 (2d6+3) slashing.
Heavy Crossbow: +2 to hit, range 100/400 ft — 5 (1d10) piercing.

BONUS ACTION — Vain Riposte
Once per turn, if he hits a creature that hasn't yet acted this round, deal an extra
1d6 damage — colpisce chi non se lo aspetta, si vanta di averlo fatto.

REACTION — Always One Step Behind
When a creature moves within 5 ft of him, he can shift 5 ft away without provoking
an opportunity attack. Codardo: non resta mai fermo a incassare.
```

Quando il corpo del cavaliere scende a 0 PF, applica i tiri salvezza contro la morte come per un PG (il tavolo potrebbe volerlo curare — è **Durstan Rial**, vittima innocente). Se muore o viene abbandonato, Caradoc emerge nella sua vera forma:

```
CARADOC — SPECTRAL FORM (Fase 2 — Villain, Action-Oriented, CR 8 ufficiale)
Medium Undead | AC 14 | HP 150
Speed 0 ft, fly 40 ft (hover) | Chaotic Evil | Languages: Common

STR -2, DEX +3, CON +0, INT +1, WIS +1, CHA +5
Resistant to acid, fire, lightning, thunder, bludgeoning/piercing/slashing from nonmagical attacks
Immune to cold, necrotic, poison damage
Immune to charmed, frightened, grappled, paralyzed, petrified, poisoned, prone, restrained
Darkvision 60 ft, Passive Perception 11

Incorporeal Movement: can move through other creatures and objects as if difficult
terrain. Takes 5 (1d10) force damage if it ends its turn inside an object.
Bound Tether: when Caradoc emerges from a host far from Dargaard Keep, he must
succeed on a DC 15 Constitution save at the end of each of his turns (automatic
success on the first turn in spectral form) or be yanked back to Dargaard Keep,
removed from the fight for the rest of the session.

ACTIONS
Possession (Recharge 6): one Humanoid Caradoc can see within 5 ft must succeed on
a DC 16 Charisma save or be possessed by him — see original stat block for full text.
Withering Touch: +8 to hit, reach 5 ft — 27 (6d8) necrotic.

BONUS ACTION — Change of Scene
Once per turn, becomes invisible until the start of his next turn or until he hits
with an attack, whichever comes first.

REACTION — Applause on Command
When a creature misses him with an attack, that creature must succeed on a DC 16
Wisdom save or be frightened of him until the end of its next turn.

VILLAIN ACTIONS (one per round, after an enemy's turn; each usable once)
1. Opener — Exit Stage: Caradoc flickers through the nearest wall or obstacle to
   anywhere within 40 ft, then makes one Withering Touch attack against whichever
   creature is not adjacent to an ally — sceglie sempre il bersaglio più isolato.
2. Control — The Crowd Goes Wild: every creature within 30 ft that can hear him
   must succeed on a DC 16 Charisma save or be frightened of him until the end of
   its next turn. Caradoc regains 10 temporary HP, one per creature frightened (max 10).
3. Ultimate — One More: Caradoc can use Possession immediately, ignoring its
   recharge, against the creature with the lowest current HP — codardo fino alla fine,
   cerca disperatamente un nuovo corpo in cui saltare piuttosto che essere distrutto.
```

**[Correzione]** Caradoc è **CR 8** nel manuale ufficiale, non ~CR5 come nella prima stesura — sottodimensionato rispetto all'originale, il che andava contro l'obiettivo di una campagna difficile. HP 75→150, AC 13→14, CHA +4→+5 (DC Possession 15→16), Withering Touch +6/21(6d6)→+8/27(6d8). Il design delle Villain Actions e il tono del testo restano gli stessi: il salto è tutto nei numeri, non nel personaggio.

**Testo da leggere (boss di capitolo — testo + Aggiunta atmosferica):**

1. *Opener — Exit Stage:*
   > Il corpo del cavaliere crolla come un vestito vuoto. Per un istante Caradoc non è da nessuna parte — poi è ovunque, un refolo gelido che vi sfiora tutti insieme, prima di richiudersi alle spalle di chi si è allontanato dagli altri.
   >
   > *[Aggiunta atmosferica]: Ha scelto voi non perché siete deboli. Vi ha scelti perché siete soli, ed è tutto ciò che gli serve sapere di una persona.*

2. *Control — The Crowd Goes Wild:*
   > *«Oh, bravi, bravissimi!»* Caradoc applaude con mani che non ci sono più, e la sua voce arriva da ogni angolo della sala insieme, sovrapposta a sé stessa.
   >
   > *[Aggiunta atmosferica]: È un suono che si infila sotto la pelle prima ancora che il cervello capisca perché fa così paura — l'applauso di qualcuno che ha già deciso come va a finire lo spettacolo.*

3. *Ultimate — One More:*
   > *«Be', è stato divertente»,* dice Caradoc, e per la prima volta nella sua voce c'è qualcosa che non è recitazione. Si volta verso il più debole di voi con un'urgenza improvvisa, affamata.
   >
   > *[Aggiunta atmosferica]: Ha smesso di godersi lo spettacolo. Ha iniziato, semplicemente, a scappare — e voi siete la porta più vicina.*

*Ordine consigliato: Opener appena emerge nella forma spettrale (mostra subito che non è più "solo" un cavaliere), Control quando il party si raggruppa per accerchiarlo, Ultimate quando fallisce un tiro di Bound Tether o è vicino a essere distrutto — è la sua ultima carta prima di essere risucchiato via o annientato.*

**Nota di continuità:** Caradoc promette ai personaggi che li rivedrà prima di svanire (che sia sconfitto o richiamato a Dargaard Keep) — nessuna modifica narrativa a questa promessa.

---

## Sessione 08 — Le Catacombe Saccheggiate e la Maledizione di Sarlamir (Livello 5 → 6)

### Guardiani Opzionali (evitabili — invariati)

```
WRAITH (Spectral Knight) (×2, se disturbati)
Medium Undead | AC 13 | HP 67 (9d8+27)
Speed 0 ft, fly 60 ft (hover) | CR 5 (1,800 XP each) | Lawful Evil

STR -2, DEX +3, CON +3, INT +1, WIS +2, CHA +2 | Skills: Perception +5
Resistant to acid, cold, fire, lightning, thunder, bludgeoning/piercing/slashing
  from nonmagical weapons
Immune to necrotic, poison damage
Immune to charmed, exhaustion, frightened, grappled, paralyzed, petrified,
  poisoned, prone, restrained
Darkvision 60 ft, Passive Perception 15

Incorporeal Movement: as Caradoc, above.
Sunlight Sensitivity: disadvantage on attack rolls and Wisdom (Perception) checks
  that rely on sight while in sunlight.

ACTIONS
Multiattack: two attacks with Life Drain.
Life Drain: +6 to hit, reach 5 ft — 21 (4d8+3) necrotic; target must succeed on a
DC 15 Constitution save or its HP maximum reduces by the damage dealt (until a
long rest), and the wraith regains HP equal to the damage dealt.
```

*Guardiani tragici, non predatori — placabili restituendo il bottino e scusandosi. Nessuna modifica: l'incontro funziona perché è evitabile, non perché è facile.*

```
SKELETAL HORSE (×2, se attaccano)
Large Undead | AC 13 (tattered barding) | HP 22 (3d10+6) | Speed 60 ft | CR 1/2

Vulnerable to bludgeoning damage
Immune to poison damage, exhaustion, poisoned condition
Darkvision 60 ft, Passive Perception 9

ACTIONS
Hooves: +6 to hit, reach 5 ft — 11 (2d6+4) bludgeoning.
```

*Placabili con armatura solamnica indossata o un successo a Wisdom (Animal Handling) — invariato.*

### Knight Sarlamir — Boss Finale del Capitolo

Il vero climax di tutto il Capitolo 4: lo scheletro rianimato che innesca l'avanzamento al 6° livello. Trattamento pieno.

**Personalità che guida il design:** Sarlamir non è malvagio per scelta — è un cavaliere onorato costretto dalla magia di Soth, che combatte perché non può fermarsi. A differenza di Gholcag (fame) e Caradoc (vanità/paura), il suo tema è la **compulsione tragica** e il fuoco del Cataclisma.

```
KNIGHT SARLAMIR (Villain, Action-Oriented, CR 7 ufficiale — HP potenziato per il party a 5)
Medium Undead | AC 18 (ancient plate armor) | HP 170
Speed 30 ft | Lawful Evil | Understands Solamnic but cannot speak
  (except compelled phrases)

STR +4, DEX +0, CON +3, INT -1, WIS +0, CHA +1
Saves: CON +6, WIS +5
Resistant to necrotic, bludgeoning/piercing/slashing from nonmagical weapons
Immune to poison, fire damage
Immune to exhaustion, frightened, poisoned condition
Darkvision 60 ft, Passive Perception 10

Relentless Undead: if reduced to 0 HP while the Cataclysm Fire still burns in the
room, can attempt a DC 15 Constitution save; on a success, rises again with 1 HP
instead of being destroyed (once per combat).

ACTIONS
Multiattack: two attacks with Flaming Sword.
Flaming Sword: +7 to hit, reach 5 ft — 11 (2d6+4) slashing + 7 (2d6) fire. Enervating
Blade: a creature hit by this attack can't regain hit points until the start of
Sarlamir's next turn.
Terrifying Presence (Recharge 5-6): every creature within 30 ft that can see him
must succeed on a DC 14 Wisdom save or be frightened for 1 minute (repeats the
save at the end of each of its turns).

BONUS ACTION — It's Not His Choice
Once per turn, if he hits with Flaming Sword, he can make one additional Flaming
Sword attack against a different creature within reach — non può fermarsi, anche
se volesse.

REACTION — Flame Retaliation
When hit by a melee attack, the attacker takes 1d6 fire damage from the violet
flame licking off his armor.

VILLAIN ACTIONS (one per round, after an enemy's turn; each usable once)
1. Opener — The Fire Remembers: the violet flame in the room flares toward him.
   Every creature within 15 ft: DC 15 Dexterity save or take 2d6 fire damage (half
   on success). Harmless-looking fire spreads across the floor within 15 ft as
   difficult terrain for the rest of the fight.
2. Control — I Cannot Stop: a compelled, sweeping strike. Every creature within
   5 ft of him: one Flaming Sword attack roll, compared against each target's AC
   individually — 11 (2d6+4) slashing + 7 (2d6) fire on a hit.
3. Ultimate — A Moment of Himself: Sarlamir stops attacking and drives his sword
   into the ground with a scream of anguish as his own will briefly resists Soth's
   compulsion, before being violently overridden. Shockwave: every creature within
   10 ft: DC 15 Constitution save or take 2d6 thunder damage and fall prone
   (half damage, no prone on a success).
```

**Testo da leggere (boss di capitolo — testo + Aggiunta atmosferica, tono trattenuto e mesto, non teatrale come Caradoc):**

1. *Opener — The Fire Remembers:*
   > Le fiamme viola sulle pareti si piegano tutte insieme verso Sarlamir, come se lo riconoscessero. Quando toccano la sua spada, la lama non si scalda — divampa.
   >
   > *[Aggiunta atmosferica]: Non sembra sorpreso. Sembra qualcuno che ha già visto questo fuoco altre trecento volte.*

2. *Control — I Cannot Stop:*
   > Il colpo parte prima che finisca di girarsi, come se la sua stessa arma non aspettasse il permesso. Non è rabbia, nel modo in cui colpisce tutto ciò che ha intorno. È qualcosa più vicino all'obbedienza.
   >
   > *[Aggiunta atmosferica]: Un cavaliere, da vivo, si sarebbe fermato per contare i nemici prima di colpire. Questo non conta più niente.*

3. *Ultimate — A Moment of Himself:*
   > Per un solo battito, Sarlamir si ferma. La lama trema a mezz'aria, e sotto l'elmo qualcosa che potrebbe essere stato un uomo sembra guardare le proprie mani.
   >
   > *[Aggiunta atmosferica]: Poi urla — non di rabbia, ma di qualcun altro che riprende il controllo — e la lama scende nella pietra.*

*Ordine consigliato: Opener al primo round (stabilisce che il fuoco della stanza è parte del combattimento, non solo scenografia), Control quando il party lo circonda, Ultimate quando è insanguinato — è l'unico momento dell'intero scontro in cui il tavolo intravede l'uomo sotto la maledizione, un attimo prima che la sua sconfitta liberi entrambi.*

**[Correzione]** Lo Skeletal Knight ufficiale su cui è basato Sarlamir è **CR 7** (2.900 XP), non CR6, con **WIS +5** ai tiri salvezza (non +3) e un'abilità distintiva — **Enervating Blade** — che qui mancava del tutto: l'ho aggiunta alla Flaming Sword. Numeri corretti sopra; design delle Villain Actions e testo invariati.

**Nota di continuità:** appena Sarlamir cade, annuncia l'avanzamento al **Livello 6** — trigger esplicito della fonte, indipendente dal conteggio XP. Nessuna modifica narrativa: Sarlamir menziona comunque la Città dei Nomi Perduti e le Northern Wastes prima di svanire.

---

## Riepilogo Modifiche

| Scontro | Originale | Adattato | Motivo |
|---|---|---|---|
| Officina di Rookledust | 10 Goblin + 1 Hobgoblin | 12 Goblin (stat block classico, iniziativa condivisa) + 1 Hobgoblin (Reaction) | scala sul quinto giocatore — niente Minion in questa campagna |
| Posto di Vedetta | 2 Baaz + 1 Kapak | 3 Baaz + 1 Kapak (+ Reaction ambusher) | scala sul quinto giocatore, coerenza col Kapak del Cap. 3 |
| Wheelwatch (soldati) | gruppi 1-8 per stanza | invariato, stat block classico anche se convergono 10+ | niente Minion in questa campagna — iniziativa condivisa se serve velocità |
| Wheelwatch (Sivak, Dragonnel+Officer) | statici | + Bonus Action/Reaction leggere | notevoli ma non boss di capitolo |
| Wheelwatch (Sivak Draconian) | CR4/HP57 ufficiale | invariato | verificato: la ricostruzione era già accurata |
| Wheelwatch (Dragonnel) | CR3 ufficiale | HP/danno a livello ~CR4 (buff dichiarato) + tratto **Flyby** aggiunto | era sovradimensionato e privo di Flyby — corretto |
| Steel Springs (guado) | 4 soldati a cavallo | 5 soldati a cavallo | scala sul quinto giocatore |
| Guardiani del cortile | 2 Undead Soldier | 3 Undead Soldier | scala sul quinto giocatore |
| **Caradoc** | CR8 ufficiale | Fase 1 leggera, Fase 2 **Villain Actions piene**, HP 75→150 (corretto da bozza iniziale ~CR5) | il vero nome-cardine della campagna dopo Kansaldi |
| Catacombe (Wraith, Skeletal Horse) | invariati | invariati | già ben calibrati come minacce opzionali/placabili |
| **Knight Sarlamir** | CR7 ufficiale (Skeletal Knight) | **Villain Actions piene**, HP 142→170, **Enervating Blade** aggiunta, WIS save +3→+5 corretto | vero boss finale, trigger del Livello 6 |

## Prossimi Passi

Capitolo 5 (The Northern Wastes, Liv. 5→8) quando Fabio conferma — Lohezet come boss atteso.
